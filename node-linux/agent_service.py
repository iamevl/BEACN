...
DEFAULT_CONFIG_PATH = Path("/etc/beacn-node/config.json")

CONFIG_PATH = Path(
    os.getenv(
        "BEACN_NODE_CONFIG",
        os.getenv(
            "NETWORK_DASHBOARD_AGENT_CONFIG",
...
LOG_PATH = Path(
    os.getenv(
        "BEACN_NODE_LOG",
        os.getenv(
            "NETWORK_DASHBOARD_AGENT_LOG",
            "/var/log/beacn-node.log",
        ),
    )
)
...
def iperf_status():
    ...
    return {
        "enabled": bool(CONFIG.get("iperf_enabled", True)),
        "available": available,
        "running": available,
        "managed": managed,
        "source": source,
