...
def resource_dir() -> Path:
    if getattr(sys, "frozen", False):
        internal_dir = application_dir() / "_internal"
...
def load_config() -> dict:
    config = {
        ...
        "hardware_helper_path": "hardware-helper.exe",
    ...
    config["iperf_path"] = resolve_resource_path(
        config["iperf_path"]
    )

    config["hardware_helper_path"] = resolve_resource_path(
        config["hardware_helper_path"]
    )
